...
void MPMSimulation::step(float dt) {
    ...
    // 3) grid update (apply forces, BCs…)
    updateGrid(dt);

    // 4) grid → particle (G2P)
    transferGridToParticles();

    // 5) move particles forward
...
{ // STEP 1 IN MPM GUIDE
    for (const auto &p : particles)
    {
        ...
        for (int i = 0; i < 3; ++i)
        { // bc we can't do 1.5 in an int loop, still loop through 3 cells and just adjust the index to be the correct neighborhood
            for (int j = 0; j < 3; ++j)
            {
                for (int k = 0; k < 3; ++k)
                {
                    ...
                    ; // converting from grid space to world space
...
void MPMSimulation::updateGrid(float dt) {
  for (auto& node : grid) {
    if (node.mass > 0.0f) {
      // apply gravity
      node.force += glm::vec3(0,-9.8f,0)*node.mass;
